...
These ship in a patch release as bug fixes rather than breaking changes because in each case the shipped behavior never matched its own documented behavior. Correcting a defect is not the same as changing intended behavior, so no deprecation cycle applies.
