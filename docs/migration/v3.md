...
::: details Why `3.0.1` and not `3.0.0`? — defects in the v2 shims
...
- **`GetPrs()` returned the inverted ratio.** It passed its two series in the wrong order, returning `base / eval`, the reciprocal of the ratio [documented for PRS](/indicators/prs). `3.0.1` returns `eval / base`, matching `ToPrs()` and v2.
...
These ship as bug fixes rather than breaking changes because the shipped behavior never matched its own documentation — see the correctness principles on [correcting a defect versus changing intended behavior](https://github.com/facioquo/stock-indicators-dotnet/blob/main/docs/PRINCIPLES.md).
