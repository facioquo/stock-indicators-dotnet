...
- **`PrsResult.PrsSma` and `RocResult.RocSma`**: Removed. v2 returned a moving average alongside these indicators; v3 has no such field. Chain `results.ToSma(smaPeriods)` onto `ToPrs()` or `ToRoc()` instead. The obsolete `GetPrs()`/`GetRoc()` tuple shims still accept an `smaPeriods` argument but discard it, and say so in their deprecation messages as of `3.0.1`.
...
### Obsolete shim signature fixes

`3.0.0` shipped two shims whose parameters were narrower than the v2 methods they stand in for, so valid v2 code did not compile. Both corrected in `3.0.1`.

- **`GetKeltner()` rejected a fractional multiplier.** It declared `int multiplier` where v2 — and `ToKeltner()` — declare `double`, so `quotes.GetKeltner(20, 2.5, 10)` failed to build with _"cannot convert from 'double' to 'int'"_. Fractional ATR multipliers are the normal case for Keltner Channels.
- **`GetStdDevChannels()` rejected a null lookback through its tuple overload.** It declared `int lookbackPeriods` where v2 declared `int?`, whose `null` means "use the whole series". The bar overload of the same method still accepts `null` and its own deprecation message tells you to use it, so the two forms disagreed.

**Do not work around either by changing the argument** — rounding a multiplier to an integer silently changes your channel widths, and substituting a fixed lookback for `null` computes a different channel. Upgrade to `3.0.1`, or move to `ToKeltner()` and `ToStdDevChannels()`, which always accepted these values.

Returned values are unchanged for every call that already compiled; these only restore call shapes v2 allowed. The signatures do change, so an assembly compiled against `3.0.0` that calls either method must be recompiled against `3.0.1` rather than swapped in place. Recompiling from source requires no edits.
...
```csharp
public class MyBar : IQuote                 // [!code --]
public class MyBar : IBar                   // [!code ++]
{
...
public static double MyVolumeSma<TBar>(
    ...
    int lookbackPeriods)
    where TBar : IBar         // [!code --]
    where TBar : class, IBar  // [!code ++]
{
